...
<#
.SYNOPSIS
  Prefetch method — JAVA/PS/LOLBin .pf surface
...
Start-SsReport 'PrefetchScanner'
$sub = 'Prefetch'
...
Write-KV 'Now' ((Get-Date).ToString('yyyy-MM-dd HH:mm:ss'))

$pfDir = Join-Path $env:SystemRoot 'Prefetch'
Write-Section 'PREFETCH DIR'
if (-not (Test-Path -LiteralPath $pfDir)) {
    Write-Bad "Prefetch missing: $pfDir"
} else {
    Write-Ok $pfDir
    $all = Get-ChildItem -LiteralPath $pfDir -Filter '*.pf' -ErrorAction SilentlyContinue
    Write-KV 'Count' ("{0}" -f @($all).Count)
    $focus = @('JAVA','JAVAW','POWERSHELL','PWSH','MSHTA','RUNDLL32','REGSVR32','CMD','WMIC','WSCRIPT','CSCRIPT','EXPLORER')
    Write-Section 'FOCUS (LOLBin / Java)'
    $shown = 0
    foreach ($pat in $focus) {
        Get-ChildItem -LiteralPath $pfDir -Filter ($pat + '*.pf') -ErrorAction SilentlyContinue |
            Sort-Object LastWriteTime -Descending | ForEach-Object {
                $shown++
                $age = (Get-Date) - $_.LastWriteTime
                $line = ("{0:yyyy-MM-dd HH:mm:ss}  {1}" -f $_.LastWriteTime, $_.Name)
                if ($age.TotalHours -le 6) { Write-Bad $line } else { Write-Ok $line }
            }
    }
    if ($shown -eq 0) { Write-Ok 'No focus Prefetch names' }

    Write-Section 'RECENT (12h)'
    $recent = @($all | Where-Object { $_.LastWriteTime -ge (Get-Date).AddHours(-12) } | Sort-Object LastWriteTime -Descending)
    if ($recent.Count -eq 0) { Write-Ok 'No .pf in last 12h' }
    else {
        $recent | ForEach-Object {
            Write-Info ("{0:yyyy-MM-dd HH:mm:ss}  {1}" -f $_.LastWriteTime, $_.Name)
        }
        Write-Warn ("Recent prefetch files: {0} (полный список в логе/CSV)" -f $recent.Count)
    }
}
Write-Info 'Deep UI: WinPrefetchView++ / JournalTrace++ for deleted .pf'
    if ($all) {
        $rows = $all | Sort-Object LastWriteTime -Descending | ForEach-Object {
            [pscustomobject]@{ LastWriteTime = $_.LastWriteTime.ToString('yyyy-MM-dd HH:mm:ss'); Name = $_.Name; FullName = $_.FullName; Length = $_.Length }
        }
        Export-SsTable $rows 'all-pf' | Out-Null
        Write-Section 'FULL PREFETCH LIST (also CSV)'
        foreach ($r in $rows) { Write-Info ("{0}  {1}" -f $r.LastWriteTime, $r.Name) }
    }
