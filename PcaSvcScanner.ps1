...
<#
.SYNOPSIS
  PcaSvc status + Program Compatibility Assistant hints
...
if (-not $svc) { Write-Bad 'PcaSvc service not found' }
else {
    Write-KV 'Status' ("{0}" -f $svc.Status)
    Write-KV 'StartType' ("{0}" -f $svc.StartType)
    if ($svc.Status -ne 'Running') { Write-Bad 'PcaSvc NOT running — embedded jar strings path weakened' }
    else { Write-Ok 'PcaSvc running' }
    if ($svc.StartType -eq 'Disabled') { Write-Bad 'PcaSvc Disabled' }
}

Write-Section 'PROCESS'
$procs = Get-CimInstance Win32_Process -ErrorAction SilentlyContinue | Where-Object { $_.Name -match '(?i)^PcaSvc|^svchost' -and $_.CommandLine -match '(?i)PcaSvc|pcaloc' }
# also look for pcaclient
$pc = Get-Process -ErrorAction SilentlyContinue | Where-Object { $_.ProcessName -match '(?i)pca' }
if ($pc) {
    foreach ($p in $pc) { Write-Ok ("Process {0} PID={1}" -f $p.ProcessName, $p.Id) }
} else { Write-Info 'No obvious pca* process name (often hosted in svchost)' }

Write-Section 'HINTS'
Write-Info 'SI Admin → PcaSvc / related svchost → Strings: jar paths, DoomsDay, mods'
Write-Info 'If cured: run Service-Enabler.ps1 then re-check BAM/Prefetch'
$reg = 'HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\AppCompatFlags\Compatibility Assistant\Store'
if (Test-Path $reg) {
    $props = (Get-Item $reg).Property
    Write-KV 'CompatStore values' ("{0}" -f @($props).Count)
    $props | Select-Object -First 80 | ForEach-Object {
        if ($_ -match '(?i)java|\.jar|minecraft|doomsday|launcher') { Write-Bad $_ }
        else { Write-Info $_ }
    }
} else { Write-Warn 'Compat Assistant Store key missing/unreadable' }
