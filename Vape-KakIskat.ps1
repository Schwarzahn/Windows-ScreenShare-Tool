...
<#
.SYNOPSIS
  Автоматизация заметки «Как искать Vape» — диск/BAM/Prefetch (SI Memory — вручную).
...
Write-Host ""
Write-Host "SCHWARZAHN · Как искать · Vape" -ForegroundColor Red
...
$needles = @('vape', 'vapev4', 'alloweditems')
$roots = @()
foreach ($r in @(
    $env:USERPROFILE,
...
foreach ($root in $roots) {
    Get-ChildItem -LiteralPath $root -Recurse -Force -File -EA SilentlyContinue |
        Where-Object {
            ...
            ($_.Extension -match '(?i)\.(exe|jar|dll|zip|7z|rar)$' -or $_.Name -match '(?i)vape')
        } |
...
if (Test-Path $pf) {
    ...
    Get-ChildItem $pf -Filter '*.pf' -EA SilentlyContinue | Sort-Object LastWriteTime -Descending | ForEach-Object {
        ...
        if ($interesting) {
            ...
            if ($ageH -le 12 -or ($n -match '(?i)vape')) { Bad $line } else { Ok $line }
            $pfShown++
        }
    }
...
Sec '6) Downloads / Temp / AppData'
foreach ($r in @((Join-Path $env:USERPROFILE 'Downloads'), $env:TEMP, $env:LOCALAPPDATA, $env:APPDATA)) {
    if (-not (Test-Path $r)) { continue }
    Get-ChildItem $r -Recurse -Force -File -EA SilentlyContinue |
        Where-Object { $_.LastWriteTime -gt (Get-Date).AddDays(-20) -and $_.Name -match '(?i)vape|alloweditems' } |
        Select-Object -First 30 | ForEach-Object {
            Bad ("{0:yyyy-MM-dd HH:mm}  {1}" -f $_.LastWriteTime, $_.FullName)
        }
}
Info 'DNS: vape.gg · OrbDiff может пометить подпись Cheat'
