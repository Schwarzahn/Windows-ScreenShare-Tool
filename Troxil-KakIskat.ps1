...
<#
.SYNOPSIS
  Автоматизация заметки «Как искать Troxil» — диск/BAM/Prefetch (SI Memory — вручную).
...
Write-Host ""
Write-Host "SCHWARZAHN · Как искать · Troxil" -ForegroundColor Red
...
foreach ($root in $roots) {
    Get-ChildItem -LiteralPath $root -Recurse -Force -File -EA SilentlyContinue |
        Where-Object {
            ...
            ($_.Extension -match '(?i)\.(exe|jar|dll|zip|7z|rar)$' -or $_.Name -match '(?i)troxil')
        } |
...
if (Test-Path $pf) {
    ...
    Get-ChildItem $pf -Filter '*.pf' -EA SilentlyContinue | Sort-Object LastWriteTime -Descending | ForEach-Object {
        ...
        if ($interesting) {
            ...
            if ($ageH -le 12 -or ($n -match '(?i)troxil')) { Bad $line } else { Ok $line }
            $pfShown++
        }
    }
