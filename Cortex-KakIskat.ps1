...
<#
.SYNOPSIS
  Автоматизация заметки «Как искать Cortex» — диск/BAM/Prefetch (SI Memory — вручную).
...
Write-Host ""
Write-Host "SCHWARZAHN · Как искать · Cortex" -ForegroundColor Red
...
$si = @('cortex', 'stubborn.website', 'blessed', 'chs/', 'Self Destruct')
...
$needles = @('cortex', 'stubborn', 'blessed', 'chs')
$roots = @()
foreach ($r in @(
    $env:USERPROFILE,
...
foreach ($root in $roots) {
    Get-ChildItem -LiteralPath $root -Recurse -Force -File -EA SilentlyContinue |
        Where-Object {
            ...
            ($_.Extension -match '(?i)\.(exe|jar|dll|zip|7z|rar)$' -or $_.Name -match '(?i)cortex|stubborn|chs')
        } |
...
if (Test-Path $pf) {
    ...
    Get-ChildItem $pf -Filter '*.pf' -EA SilentlyContinue | Sort-Object LastWriteTime -Descending | ForEach-Object {
        ...
        if ($interesting) {
            ...
            if ($ageH -le 12 -or ($n -match '(?i)cortex|stubborn|chs')) { Bad $line } else { Ok $line }
            $pfShown++
        }
    }
...
Sec '6) .minecraft / DNS hint'
$mc = Join-Path $env:APPDATA '.minecraft'
if (Test-Path $mc) {
    Get-ChildItem (Join-Path $mc 'mods') -File -EA SilentlyContinue | ForEach-Object {
        foreach ($n in $needles) {
            if ($_.Name.IndexOf($n, [StringComparison]::OrdinalIgnoreCase) -ge 0) {
                Bad ("mods  {0}" -f $_.FullName)
            }
        }
    }
    Ok (".minecraft: {0}" -f $mc)
} else { Warn 'Нет .minecraft' }
Info 'DNS/браузер: stubborn.website (BrowsingHistoryView++ / Dnscache в SI)'
